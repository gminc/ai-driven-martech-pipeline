...
**商品圖怎麼來？** 這是一個虛構品牌，沒有實拍素材可用，所以商品與情境照都是在本機用 [Pollinations.ai](https://pollinations.ai/)（Flux 模型，免金鑰）生成後，壓成 JPEG 直接放進 `static/img/`：商品與情境照 886 × 615、約 38–100 KB，全幅主視覺 325 KB（手機版另備一張 78 KB 的窄版），整個 `static/img/` 合計約 1 MB，沒有任何外部圖床。圖片出處標在每一頁最上方的展示站說明列，因為用生成圖假裝成實拍照片並不誠實，而這件事跟「網站要有真實感」並不衝突。

兩個實務細節：所有商品圖都統一成同一個尺寸，`<img>` 的 `width`／`height` 也照著寫，瀏覽器才會在圖片載入前先保留正確版位，不會等圖片解碼完才把底下的內容往下推（CLS）。另外，Day 14 之後要做素材特徵抽取時，這種「同一組提示詞產出的圖」變異度偏低，到時候會再補一批風格差異更大的素材。
