...
今天產生的事件不是做完就結束，Day 05 的合成器會沿用同一組商品 ID 與事件欄位，讓合成資料與真實事件對得起來；Day 08 的多觸點歸因則直接拿今天匯出到 BigQuery 的事件表當輸入，今天等於是在替後面兩週的分析鋪第一段軌道。
...
這次搭建的網站具備完整的電商動線，今天先專注在伺服器端的金流驗證與電商事件，其餘頁面則是為了後續實戰預留資料：Day 08 的多觸點歸因需要完整的瀏覽與結帳路徑，Day 19 也需要實際的 Landing Page 來比對廣告素材的一致性。
...
| `/product/<商品 ID>` | 商品詳情：多圖、材質規格、洗滌方式 | 商品檢視、Day 16 素材特徵抽取 |
| `/about` | 品牌故事、三大工藝主張、常見問題 | 瀏覽深度、跳出率對照 |
| `/lp/<活動代號>` | 活動頁面 | Day 19 Landing Page 與廣告素材一致性 |
...
商品圖的部分，這是一個虛構品牌沒有實拍素材可用，所以商品與情境照都是在本機用 [Pollinations.ai](https://pollinations.ai/)（Flux 模型，免金鑰）和 Gemini 生成後壓成 JPEG 放進 `static/img/`，商品與情境照統一 886 × 615、38 到 100 KB，另有兩張全幅主視覺（桌機 1920 × 1072、手機 900 × 502），整個資料夾合計約 1 MB，沒有任何外部圖床，圖片出處標在每一頁最上方的展示站說明列，Day 16 要做素材特徵抽取時，這種同一組提示詞產出的圖變異度偏低，到時候會再補一批風格差異更大的素材。
...
`CustomField3` 與 `CustomField4` 是整篇文章最重要的兩格，有了它們，Cloud Logging 裡的每一筆付款紀錄都能對應到 GA4 的哪一個訪客、來自哪一個廣告活動，Day 08 的多觸點歸因才有辦法把「廣告花費」和「實際成交」接起來。
...
`item_list_id` 會隨列表位置變化，本站有三種取值：首頁的 `home_all`、活動頁的 `lp_<活動代號>`、商品頁下方相關商品的 `related_<商品 ID>`，之後就能回答「從活動頁點進去的人最後買了什麼」這種問題，`view_promotion` 與 `creative_name` 則是 Day 19 比對廣告素材與 Landing Page 的接點。
...
| `event_timestamp` | 微秒級時間戳 | Day 08 排出使用者的接觸順序 |
...
`event_params` 與 `items` 都是巢狀結構，查詢時要用 `UNNEST`，這是 GA4 匯出資料最容易卡住新手的地方，Day 07 會一併處理。
